...
import { availabilityOf, isUsable, needsDownload, resolveGlobal } from "./ai/availability.ts";
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === "sync" && Object.keys(changes).some(isPresetKey)) void buildMenus();
});
  ...
  if (message?.type === "GET_STATE") {
    void (async () => {
      ...
      const response: StateResponse = {
        state: isUsable(state) ? "ready" : needsDownload(state) ? "downloadable" : "unavailable",
      };
